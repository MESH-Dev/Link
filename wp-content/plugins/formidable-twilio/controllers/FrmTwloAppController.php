...
class FrmTwloAppController{
    ...
    public static function process_notification(){
		if ( ! class_exists( 'FrmAppHelper' ) ) {
			// exit now if Formidable is not active
			return;
		}
		...
		$phone_number = santitize_text_field( $_REQUEST['From'] );
		$vote = santitize_text_field( $_REQUEST['Body'] );
    ...
    */
	...
	private static function format_number( $num ) {
		...
		if ( strpos( $num, '1' ) !== 0 && strlen( $num ) == 10 ) {
			$num = '1' . $num;
        }
        $num = '+' . $num;
    ...
    */
    private static function save_vote($phone_number, $vote, $form_id){
        ...
        if($allowed){
            ...
            $fields = FrmFields::getAll( $form_id );
			...
			if ( $id = FrmEntry::create( $values ) ) {
				$message = __( 'Thank you, your vote has been recorded.', 'frmtwlo' );
